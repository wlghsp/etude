...
INSERT INTO quest_set (title, description, sandbox_type) VALUES
  ...
  ('리눅스 네트워크/파일 전송',        'curl, ping, scp, rsync 등 네트워크 확인과 서버 간 파일 전송을 실습합니다.', 'linux-ssh'),
...
INSERT INTO quest (quest_set_id, order_index, title, description, hint, solution, setup_cmd, grade_cmd) VALUES
  ...
  (3,  5, 'SSH로 원격 파일 복사하기',
   ...
   '["sh", "-c", "curl -s -o /tmp/index.html http://example.com"]',
   '["test", "-f", "/tmp/remote_copy.html"]'),

  (3,  6, 'rsync로 디렉토리 동기화하기',
   '/tmp/sync_src 디렉토리를 만들고 파일을 하나 넣은 뒤, rsync로 /tmp/sync_dst 에 동기화하세요.',
   '1) mkdir /tmp/sync_src 로 디렉토리 생성 2) touch /tmp/sync_src/file.txt 로 파일 생성 3) rsync -av /tmp/sync_src/ /tmp/sync_dst/ 로 동기화',
   'mkdir /tmp/sync_src
touch /tmp/sync_src/file.txt
rsync -av /tmp/sync_src/ /tmp/sync_dst/',
   NULL,
