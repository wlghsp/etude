...
**한 일**: [testing_layers.md](../testing/testing_layers.md) 작성 — 엔티티 단위 테스트(mock
없음), 도메인 서비스(mockk로 리포지토리 목킹), 컨트롤러 통합 테스트(Testcontainers 실제 DB),
리포지토리 구현체(아직 별도 테스트 없음) 4단으로 명문화. `docs/guides/`가 아니라 `docs/testing/`에
둔 이유는 이게 순서대로 밟는 구현 가이드가 아니라 계속 참고하는 원칙 문서이기 때문.
...
- **위치**: `docs/domain/` 새 폴더로 결정. `docs/glossary/`(기술 개념 설명)와는 성격이 달라
  분리했다.
- **형식**: splearn의 3열 용어사전 표를 그대로 채택. "속성/행위/규칙" 애그리게잇 템플릿도 채택 —
  CLAUDE.md의 "명세 선행(Spec-First)" 원칙과 잘 맞물린다.
- **다이어그램**: `.drawio` 대신 Mermaid(`erDiagram`)로 결정.
- **한 일**: [용어사전.md](../domain/용어사전.md), [도메인모델.md](../domain/도메인모델.md) 작성.

### 2-2. 적용 범위 — 전체가 아니라 핵심 애그리게잇부터 (완료)
...
(Step 3에서 막 구현 완료)부터 시범 도입했다. 다음 Step(Step 4 이후) 진행 시 새로 설계하는
애그리게잇에 자연스럽게 확장한다.

### 2-3. 코드-문서 연결 방식 (보류 — 소스 수정 필요, 사용자 직접 진행)

splearn처럼 코드에 문서를 가리키는 주석은 없이 커밋 단위로만 동기화하는 방식도 가능하지만,
Etude에서는 엔티티 파일 상단에 관련 문서 경로를 한 줄 주석으로 남기는 걸 보완책으로 고려한다 —
splearn 분석에서 "코드-문서 간 명시적 링크 부재"를 약점으로 짚었기 때문. 예: `QuestSet.kt` 상단에
`// docs/domain/도메인모델.md 참고`. 소스 파일 수정이 필요해 AI가 대신 하지 않는다(CLAUDE.md
원칙) — 필요하면 직접 추가.
