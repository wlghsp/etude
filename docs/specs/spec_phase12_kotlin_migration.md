...
원칙: 의존성이 적은 것부터, TDD(실패하는 테스트 → 구현 → 리팩터링)로 검증 가능한 단위로 쪼갠다.
각 Step은 **별도 가이드 파일**로 제공한다 — 한 파일에 전체를 담지 않는다.
Step마다 끝에 테스트 실행(`./gradlew test`)과 필요 시 `curl`/브라우저로 기존 Node 백엔드와 동작을 비교하며 진행한다.
