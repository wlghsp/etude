...
Step 6-1 진행 중 이 방침의 비용이 예상보다 크다는 게 드러났다 — 브라우저로 실제 로그인부터
확인하는 수동 검증 자체가 Step 10 전까지 불가능해, 백엔드만으로 완결되지 않는 기능(WebSocket
프로토콜 등)의 실물 검증이 어려웠다. 그래서 Step 10의 "프론트 API 모듈 전체 전환"을 통째로
당기지 않고 **`frontend/src/api/`에 얇은 어댑터 계층만 추가**해 이 문제를 조기에 해소하는
방법을 [guide_phase12_frontend_api_adapter.md](../guides/guide_phase12_frontend_api_adapter.md)
로 정리했다. 컴포넌트/훅 코드는 건드리지 않으며, Step 10에서는 이 어댑터가 이미 적용돼 있다는
전제로 나머지(엔드포인트 URL 변경이 있다면 그 갱신, 전체 시나리오 최종 회귀 검증)만 다루면 된다.
