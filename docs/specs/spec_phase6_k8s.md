...
namespace 격리로 세션 간 리소스(Pod/Deployment/Service)는 분리된다.
단, 클러스터를 공유하므로 아래는 격리되지 않는다:

- 클러스터 레벨 명령 (`kubectl get nodes` 등) — 모든 세션이 동일한 결과를 봄
- 노드 리소스 (CPU/메모리) — 한 세션의 무거운 작업이 다른 세션에 영향 가능
- `--all-namespaces` 옵션 — 다른 세션의 namespace가 보임

기초 실습 수준에서는 이 정도 격리로 충분하다.
완전한 격리(사용자별 클러스터)는 리소스 문제로 향후 검토.
...
| 파일 | 수정 내용 |
|------|-----------|
| `terminal.ts` | `SandboxType` 타입 제거 (DB FK가 이미 검증, 코드 중복), k8s 분기 처리, namespace 생성/삭제 |
...
```
KUBECONFIG_PATH=/Users/{username}/.kube/config-etude
K3D_NETWORK=k3d-etude
```

`K3D_NETWORK`: k3d 클러스터 이름이 바뀌면 `k3d-{클러스터명}` 규칙으로 함께 변경.
...
- [x] init.sql — sandbox k8s binds, quest_set 6, quest 세트 6 (10개) 추가
