...
Unix 소켓 파일(`/var/run/docker.sock`)이 생성될 때까지 컨테이너 내에서 대기한다.
`docker info` exec 루프보다 안정적이고 dockerd가 완전히 뜨기 전에 소켓이 생성되는 타이밍 문제가 없다.
...
컨테이너 내 sh가 소켓 파일 생성을 감지하면 종료 → `exec.inspect().Running === false` → resolve.
