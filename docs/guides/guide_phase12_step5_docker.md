...
> 동일하게 환경변수(`DOCKER_HOST` 등)나 OS 기본 소켓(`/var/run/docker.sock`)을 자동으로
> 찾습니다 — 원본이 별도 설정 없이 `new Docker()`만 호출하는 것과 대응됩니다.
...
> `withLabelFilter`가 아닐 수 있으니, 실제 작성 시 `docker-java-core:3.4.0`의
...
class SandboxConfigService(
    ...
    private fun parseBinds(json: String): List<String> =
        // JSON 배열 문자열(`["a:b:ro"]`)을 파싱 — Jackson ObjectMapper 등 이미 프로젝트에
        // 있는 JSON 라이브러리를 사용
        TODO("ObjectMapper로 List<String> 파싱")
}
```

> `kubeconfigPath`를 생성자로 주입받는 이유는 원본이 `process.env.KUBECONFIG_PATH ??
> \`${process.env.HOME}/.kube/config\``로 환경변수를 직접 읽는 부분을, 테스트에서 값을 고정할
> 수 있도록 `application.yml`의 `@Value`로 옮기기 위함입니다 — `SandboxConfigService`를
> `@Service` 빈으로 등록할 때 아래처럼 구성합니다.

```kotlin
@Service
class SandboxConfigService(
    private val sandboxConfigRepository: SandboxConfigRepository,
    @Value("\${etude.kubeconfig-path:#{systemProperties['user.home'] + '/.kube/config'}}")
    private val kubeconfigPath: String,
)
```

> 위처럼 `@Value` 기본값 SpEL로 `HOME` 환경변수 부재 시 폴백까지 표현할 수 있지만, SpEL이
> 번잡하면 `application.yml`에 `etude.kubeconfig-path: ${KUBECONFIG_PATH:${user.home}/.kube/config}`
> 를 정의하고 `@Value("\${etude.kubeconfig-path}")`로 단순하게 주입받아도 됩니다 — 어느 쪽이든
> "환경변수 → 기본값" 폴백 순서만 원본과 같으면 됩니다.
...
**수동 검증** — Docker Desktop을 켠 채로 확인합니다:
...
기존 Node.js 백엔드(`backend/`)로 같은 시나리오를 재현해 정리 시점과 결과가 동일한지 비교합니다.
